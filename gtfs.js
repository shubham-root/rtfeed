...
exports.GTFS = class {
  constructor() {
    ...
    this.db.all(`SELECT t.route_id AS route_id, t.service_id AS service_id,
        ...
        WHERE t.service_id = c.service_id AND c.date = 20180714
        ORDER BY departure_date, departure_time`), [], (err, rows) => {
	  if (err) {
	    throw err;
	  }
	  rows.forEach((row) => {
